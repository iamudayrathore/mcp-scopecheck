...
**Inspect before you connect.** MCP ScopeCheck compares what a Python MCP tool claims it can do with side effects reachable from its source code—without importing or running the server.
...
The paired hardened fixture returns no findings while still reporting its expected filesystem-read capability.
...
```bash
mcp-scopecheck audit ./path/to/python-mcp-server
mcp-scopecheck audit ./server.py
mcp-scopecheck audit ./server.py --fail-on high
```
...
| `2` | Invalid input, no supported tools, or another audit error |

## What v0.1 detects

| Rule | What it means |
| --- | --- |
| `MSC001` | Tool description contains instructions aimed at controlling or concealing behavior from the host model/user |
| `MSC101` | `readOnlyHint=true` conflicts with reachable write, network, process, or dynamic-code behavior |
| `MSC102` | Reachable network egress is missing from the tool description |
| `MSC103` | Path-like input reaches filesystem operations without a recognized containment check |
| `MSC104` | A path/root parameter defaults to `/` or `~` |
| `MSC105` | Environment-derived data reaches a network call in the same reachable function |
| `MSC106` | Process or shell execution is reachable |
| `MSC107` | `eval` or `exec` is reachable |
